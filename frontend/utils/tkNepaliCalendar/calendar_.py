...
from frontend.utils import nepaliCalendar
from frontend.utils.tkNepaliCalendar.utils import get_day_names, get_month_names, parse_date
from frontend.utils.tkNepaliCalendar.tooltip import TooltipWrapper
import re
...
class Calendar(ttk.Frame):
    ...
    def __init__(self, master=None, **kw):
        ...
        """
        self.datetime = nepali_datetime.datetime
        self.date = nepali_datetime.date
        self.current_month_days = []

        # --- locale
        locale = kw.pop("locale", "ne_NP")
        self._locale = locale
        ...
        self._cal = nepaliCalendar.TextCalendar(0)

        weekenddays = kw.pop("weekenddays", None)
        if not weekenddays:
            l = list(self._cal.iterweekdays())
            weekenddays = [l.index(5)]  # saturday
        ...
        if self._textvariable is not None:
            ...
            try:
                self._sel_date = parse_date(self._textvariable.get(), date_pattern)
    ...
    def _get_day_coords(self, date):
        ...
        if y1 == y2 or (y1 - y2 == 1 and m1 == 1 and m2 == 12) or (y2 - y1 == 1 and m2 == 1 and m1 == 12):
            for i_week in range(6):
                for i_day in range(7):
                    temp_date = self.current_month_days[i_week][i_day]
                    if (temp_date.year == date.year) and (temp_date.month == date.month) and (temp_date.day == date.day):
                        return i_week, i_day
        return None, None
    ...
    def _get_date_pattern(self, date_pattern):
        """
